    ...
    <div className="flex gap-5" role="tablist">
      {VIEWS.map(([v, label]) => (
        <Link key={v} role="tab" aria-selected={current === v} href={buildDashboardHref({ view: v, status: undefined, floor: undefined }, params)}
          className={cn("label pb-1 border-b-2 border-transparent hover:text-ink focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-accent", current === v && "text-ink border-accent")}>
          {label} View
        </Link>
      ))}
    </div>
