...
export function SettleButton({ reservationId, balance }: { reservationId: string; balance: number }) {
  ...
  return (
    <div className="flex items-center gap-3 justify-end">
      ...
      <Button disabled={!settled} loading={pending} onClick={() => start(async () => { const r = await settleReservation(reservationId); if (r.ok) { router.push(`/reservations/${reservationId}`); router.refresh(); } else setError(r.message); })}>
        Settle & Check Out
      </Button>
    </div>
  );
